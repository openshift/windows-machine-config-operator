...
func main() {
	...
	if err != nil {
		setupLog.Error(err, "failed to create client for TLS configuration")
		os.Exit(1)
	}

	tlsProfile, err := tlspkg.FetchAPIServerTLSProfile(ctx, cl)
	if err != nil {
		setupLog.Error(err, "failed to fetch TLS profile from APIServer")
		os.Exit(1)
	}

	tlsAdherence, err := tlspkg.FetchAPIServerTLSAdherencePolicy(ctx, cl)
	if err != nil {
		setupLog.Error(err, "failed to fetch TLS adherence policy from APIServer")
		os.Exit(1)
	}

	setupLog.Info("TLS configuration loaded",
		"minVersion", tlsProfile.MinTLSVersion,
		"ciphers", len(tlsProfile.Ciphers),
	...
	tlsWatcher := &tlspkg.SecurityProfileWatcher{
		...
		OnAdherencePolicyChange: func(ctx context.Context, oldPolicy, newPolicy configv1.TLSAdherencePolicy) {
			setupLog.Info("TLS adherence policy changed, pod will restart",
				"oldPolicy", oldPolicy,
				"newPolicy", newPolicy)
			os.Exit(0) // Graceful exit - Kubernetes will restart pod
		},
	...
	setupLog.Info("starting manager with TLS security profile enforcement")
