...
func main() {
	...
	// Setup WICD admission webhook server for node-specific access control
	// This webhook enforces that WICD certificate-based users can only modify their own nodes
	webhookServer := webhook.NewServer(mgr, "/tmp/k8s-webhook-server/serving-certs")
	if err := webhookServer.SetupWithManager(mgr); err != nil {
		setupLog.Error(err, "unable to setup WICD admission webhook with manager")
		os.Exit(1)
	}
