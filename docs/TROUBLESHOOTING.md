...
- WICD service (`windows-instance-config-daemon`), and windows-exporter (`windows-exporter`) however, run as standalone processes with no
  `kube-log-runner` wrapper. They write their own log files directly under `C:\var\log\wicd\` and `C:\var\log\windows_exporter\windows_exporter.log` respectively.
...
ip-10-0-138-252.us-east-2.compute.internal windows_exporter/
