...
KUBELET_GIT_VERSION=v1.29.2+2eba5a9
KUBE-PROXY_GIT_VERSION=v1.29.0+948d02f
CONTAINERD_GIT_VERSION=v1.7.10
...
manifests: controller-gen ## Generate WebhookConfiguration, ClusterRole and CustomResourceDefinition objects.
	$(CONTROLLER_GEN) rbac:roleName=windows-machine-config-operator crd webhook paths="{./cmd/..., ./internal/controller/..., ./pkg/...}" output:crd:artifacts:config=config/crd/bases
...
generate: controller-gen ## Generate code containing DeepCopy, DeepCopyInto, and DeepCopyObject method implementations. Must be run when adding or changing a CRD.
	$(CONTROLLER_GEN) object:headerFile="hack/boilerplate.go.txt" paths="{./cmd/..., ./internal/controller/..., ./pkg/...}"
