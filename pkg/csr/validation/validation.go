...
type CertificateType struct {
	...
	GroupName string
	// AllowedUsages are the permitted key usages for this certificate type
	AllowedUsages []certificatesv1.KeyUsage
	// AllowedUsagesNoRSA are the permitted key usages for non-RSA certificates
...
func (v *CSRValidator) ValidateCSR(ctx context.Context, csr *certificatesv1.CertificateSigningRequest) error {
	...
	// Validate node exists if required
...
func (v *CSRValidator) validateCSRSpec(csr *certificatesv1.CertificateSigningRequest, parsedCSR *x509.CertificateRequest) error {
	...
	// Validate key usages
	if !v.hasValidUsages(csr.Spec.Usages) {
...
func (v *CSRValidator) hasValidUsages(csrUsages []certificatesv1.KeyUsage) bool {
	if len(csrUsages) == 0 {
		return false
	}

	// Check against allowed usages (with RSA support)
	if v.hasUsages(csrUsages, v.certType.AllowedUsages) {
		return true
	}

	// Check against allowed usages (without RSA support)
	if len(v.certType.AllowedUsagesNoRSA) > 0 && v.hasUsages(csrUsages, v.certType.AllowedUsagesNoRSA) {
		return true
	}
...
// hasUsages verifies if the CSR usages match the expected usages exactly
func (v *CSRValidator) hasUsages(csrUsages, expectedUsages []certificatesv1.KeyUsage) bool {
	if len(csrUsages) != len(expectedUsages) {
		return false
	}

	usageMap := make(map[certificatesv1.KeyUsage]struct{})
	for _, usage := range expectedUsages {
		usageMap[usage] = struct{}{}
	}

	for _, usage := range csrUsages {
