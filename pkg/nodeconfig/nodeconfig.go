...
func (nc *nodeConfig) Configure() error {
	...
	err = func() error {
		...
		migratedAddr, err := nc.addSSHHostKeyAnnotation(context.TODO(), annotationsToApply)
		if err != nil {
		...
		if migratedAddr != "" {
			...
			if err := validator.RemoveHostKey(context.TODO()); err != nil {
...
func (nc *nodeConfig) createBootstrapFiles() error {
	...
	filePathsToContents[windows.BootstrapCaCertPath], err = nc.generateBootstrapCaCert()
	if err != nil {
		return err
	}
...
func (nc *nodeConfig) generateBootstrapCaCert() (string, error) {
