...
func TestHostKeyValidator_MigrateFromConfigMap(t *testing.T) {
	scheme := runtime.NewScheme()
	require.NoError(t, core.AddToScheme(scheme))

	hostKey := generateTestHostKey(t)
	keyB64 := base64.StdEncoding.EncodeToString(hostKey.Marshal())

	// Create ConfigMap with host key
	cm := &core.ConfigMap{
		ObjectMeta: meta.ObjectMeta{
			Name:      SSHHostKeysConfigMap,
			Namespace: "openshift-windows-machine-config-operator",
	...
	// Create Node without host key annotation
	node := &core.Node{
		ObjectMeta: meta.ObjectMeta{
			Name: "test-node",
		},
	...
	fakeClient := fake.NewClientBuilder().WithScheme(scheme).WithObjects(cm, node).Build()
	ctx := context.Background()

	// Create validator with both nodeName and address (migration scenario)
	validator := NewHostKeyValidator(fakeClient, "openshift-windows-machine-config-operator", "test-node", "10.1.42.1")

	// Perform migration
	err := validator.MigrateFromConfigMap(ctx)
	require.NoError(t, err, "migration should succeed")

	// Verify key was stored to Node annotation
	var updatedNode core.Node
	err = fakeClient.Get(ctx, client.ObjectKey{Name: "test-node"}, &updatedNode)
	require.NoError(t, err)

	storedKey, exists := updatedNode.Annotations[SSHHostKeyAnnotation]
	require.True(t, exists, "host key annotation should exist after migration")
	assert.Equal(t, keyB64, storedKey, "migrated key should match original")

	// Verify ConfigMap entry was cleaned up
	var updatedCM core.ConfigMap
	err = fakeClient.Get(ctx, client.ObjectKey{
		Namespace: "openshift-windows-machine-config-operator",
		Name:      SSHHostKeysConfigMap,
	}, &updatedCM)
	require.NoError(t, err)
	assert.NotContains(t, updatedCM.Data, "10.1.42.1", "ConfigMap entry should be cleaned up after migration")
}
