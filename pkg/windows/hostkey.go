...
// MigrateFromConfigMap migrates SSH host key from ConfigMap to Node annotation
// Used by Node controller to handle Machine API nodes where SSH happened before Node existed
func (v *HostKeyValidator) MigrateFromConfigMap(ctx context.Context) error {
	if v.store == nil {
		return nil
	}
	store, ok := v.store.(*nodeAnnotationHostKeyStore)
	...
	if store.address == "" || store.nodeName == "" {
		return nil
	}

	// Read key from ConfigMap
	key, err := store.getKeyFromConfigMap(ctx, store.address)
	if err != nil {
		return fmt.Errorf("failed to read key from ConfigMap: %w", err)
	}
	if key == nil {
	...
	// Store to Node annotation
	if err := store.StoreHostKey(ctx, "", key); err != nil {
		return fmt.Errorf("failed to store key to Node annotation: %w", err)
	}

	store.log.Info("migrated SSH host key from ConfigMap to Node annotation", "address", store.address)
	return nil
}
...
func (s *nodeAnnotationHostKeyStore) storeKeyToConfigMap(ctx context.Context, address string, key ssh.PublicKey) error {
	...
	patchData := fmt.Sprintf(`[{"op":"add","path":"/data/%s","value":"%s"}]`, address, keyB64)
	err := s.client.Patch(ctx, cm, client.RawPatch(types.JSONPatchType, []byte(patchData)))
	if err == nil {
		return nil
	}
	...
	if apierrors.IsNotFound(err) {
		...
		if err := s.client.Create(ctx, cm); err != nil {
			...
			patchErr := s.client.Patch(ctx, cm, client.RawPatch(types.JSONPatchType, []byte(patchData)))
			if patchErr != nil {
