...
func TestValidatePublicKey(t *testing.T) {
	...
	testCases := []struct {
		...
		{"Weak RSA", weak1024RSAPub, true, fmt.Sprintf("RSA key size is %d bits, which is considered weak. A minimum of 2048 bits is recommended", 1024)},
		{"Strong ECDSA", strongECDSAPub, false, ""},
		{"Ed25519", ed25519SSHPub, false, ""},
		{"DSA", dsaPub, true, "DSA keys are deprecated and considered weak. Please use RSA, ECDSA, or Ed25519"},
	}
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.wantErr && err.Error() != tc.errMsg {
				t.Errorf("ValidatePublicKey() error msg = %q, want %q", err.Error(), tc.errMsg)
			}
		})
	}
}
