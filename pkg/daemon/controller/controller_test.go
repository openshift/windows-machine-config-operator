...
func TestHTTPProxyCheck(t *testing.T) {
	var tests = []struct {
		...
		},
		{
			...
			check: HTTPProxyCheck{
				Proto: "https",
				Host:  "api-int.mohashai-aws-proxy0.devcluster.openshift.com",
			}, // Expected not to go via proxy, in NO_PROXYZ
			expectWarnings: false,
		},
	}
	fmt.Println("starting setting")
	resetProxyEnv(t)
...
func resetProxyEnv(t *testing.T) {
	...
	t.Setenv("NO_PROXY", "example.com,10.0.0.0/8,2001:db8::/48,.cluster.local,.svc,.us-east-2.compute.internal,10.0.0.0/16,10.128.0.0/14,127.0.0.1,169.254.169.254,172.30.0.0/16,api-int.mohashai-aws-proxy0.devcluster.openshift.com,localhost")
