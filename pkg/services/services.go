...
func kubeProxyConfiguration(debug bool) servicescm.Service {
	...
	cmd := fmt.Sprintf("%s --windows-service --proxy-mode=kernelspace --feature-gates=WinOverlay=true,WinDSR=true "+
		"--hostname-override=NODE_NAME --kubeconfig=%s --cluster-cidr=NODE_SUBNET --log-dir=%s --logtostderr=false "+
		"--network-name=%s --source-vip=ENDPOINT_IP --enable-dsr=true", windows.KubeProxyPath, windows.KubeconfigPath,
