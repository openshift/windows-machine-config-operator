...
func NewConfigMapReconciler(mgr manager.Manager, clusterConfig cluster.Config, watchNamespace string) (*ConfigMapReconciler, error) {
	...
	svcData, err := services.GenerateManifest(clusterConfig.Network().GetServiceCIDR(),
		clusterConfig.Network().VXLANPort(), string(clusterConfig.Platform()), ctrl.Log.V(1).Enabled())
	if err != nil {
