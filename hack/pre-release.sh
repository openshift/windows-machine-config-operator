...
github_update() {
  ...
  generate_commit "$commit_message" build bundle Makefile config Containerfile Containerfile.bundle
...
gitlab_update() {
 if [ "$#" -ne 3 ]; then
  ...
  updated_version=$3
  OCP_version=$(get_OCP_version $wmco_version)
  updated_OCP_version=$(get_OCP_version $updated_version)
  rhel_version=$(get_rhel_version $OCP_version)
  ...
  if [[ $stream == "x" ]]; then
    # Example: matches "branch: rhaos-4.13-rhel-9"
    sed -i "s/branch: rhaos-$OCP_version-rhel-$rhel_version/branch: rhaos-$updated_OCP_version-rhel-$rhel_version/" versions/release-$updated_version/product.yml
   
    #Example: matches "ref: rhaos-413-rhel-9"
    sed -i "s/ref: rhaos-$OCP_version-rhel-$rhel_version/ref: rhaos-$updated_OCP_version-rhel-$rhel_version/" versions/release-$updated_version/product.yml

    #Example: matches OSE-4.11-RHEL-8
    sed -i "s/OSE-$OCP_version-RHEL-$rhel_version/OSE-$updated_OCP_version-RHEL-$rhel_version/" versions/release-$updated_version/product.yml
  fi 
