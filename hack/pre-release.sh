...
# check to see if the opeator-sdk version is correct 
...
if [[ "$operator_sdk_current_version" != "$OPERATOR_SDK_VERSION" ]]; then
  echo "operator-sdk $OPERATOR_SDK_VERSION must be installed."  
  echo "exiting."
...
commit_message="[$base_branch] Update version to $WMCO_VERSION
