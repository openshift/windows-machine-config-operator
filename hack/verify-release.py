...
3. GIT TAG EXISTS
   Source:    GitHub API — GET /git/ref/tags/v{version}
...
def _mentions_version(text: str, version: str) -> bool:
    """True if `version` appears in `text` and is not part of a longer version string."""
...
def check_advisory_version_match(image, all_versions):
    ...
    if not _mentions_version(text, version):
    ...
    wrong = sorted(v for v in all_versions if v != version and _mentions_version(text, v))
    if wrong:
        return False, (
...
def check_git_tag_exists(image, _all_versions):
    ...
    version = image["version"]
    tag = f"v{version}"
    url = f"{GITHUB_API}/git/ref/tags/{tag}"
    ...
    if isinstance(ref, list):
        exact = [r for r in ref if r.get("ref") == f"refs/tags/{tag}"]
        ref = exact[0] if exact else {}
    obj = ref.get("object", {})
    sha = obj.get("sha", "")
...
def check_advisory_yaml(image, _all_versions):
    ...
    for img_entry in images:
        if not isinstance(img_entry, dict):
            failures.append(f"image entry is {type(img_entry).__name__}, expected mapping")
            continue
        component = img_entry.get("component", "unknown")
        tags = img_entry.get("tags", [])
        if not isinstance(tags, list):
            failures.append(f"{component} tags field is {type(tags).__name__}, expected list")
            continue

        # Check 4: each image's tags list must include a "v{version}-{build}" entry.
        if not any(isinstance(t, str) and t.startswith(versioned_tag_prefix) for t in tags):
            failures.append(
...
def _git_tag_sha(version) -> str:
    ...
    headers = _github_headers()
    try:
        resp = _get(f"{GITHUB_API}/git/ref/tags/{tag}", headers=headers, timeout=15)
        if resp.status_code != 200:
            return ""
        ref = resp.json()
        if isinstance(ref, list):
    ...
    while True:
        ...
        if resp.status_code != 200:
            raise requests.HTTPError(
                f"GitHub tag listing failed on page {page}: HTTP {resp.status_code} "
